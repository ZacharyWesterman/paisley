...
- `atan2(x: number, y: number) -> any`
  - Calculate the signed arctangent of y/x.
- `ceil(x: number) -> any`
  - Round up to the nearest integer.
- `clamp(value: number, min: number, max: number) -> any`
...
- `lerp(ratio: number, start: number, end: number) -> any`
  - Linearly interpolate between two numbers.
- `log(base: number [, value: number]) -> any`
...
- `smoothstep(value: number, min: number, max: number) -> any`
...
- `keys(obj: object[any]) -> any`
  - List an object's keys.
- `object(list: array[any]) -> any`
...
- `pairs(data: object[any]|array[any]) -> any`
...
## Strings
- `beginswith(search: string, substring: string) -> any`
  - Check if the search string begins with the given substring.
- `camel(text: string) -> any`
  - Capitalize the first letter of every word.
- `endswith(search: string, substring: string) -> any`
  - Check if the search string ends with the given substring.
- `file_glob(glob_pattern: string) -> any`
...
- `filter(text: string, valid_chars: string) -> any`
...
- `hex(value: number) -> any`
  - Convert a number to a hexadecimal string.
- `join(list: array[any], delim: string) -> any`
  - Join an array into a single string with a delimiter between elements.
- `lower(text: string) -> any`
  - Convert a string to lowercase.
- `lpad(text: string, pad_char: string, width: number) -> any`
  - Left-pad a string with a given character.
- `match(text: string, pattern: string) -> any`
  - Get the first substring that matches the given pattern. If no match is found, null is returned.
- `matches(text: string, pattern: string) -> any`
  - Get all substrings that match the given pattern.
- `numeric_string(number: number, base: number, pad_width: number) -> any`
...
- `replace(text: string, search: string, replace: string) -> any`
  - Replace all occurrences of a substring.
- `rpad(text: string, pad_char: string, width: number) -> any`
  - Right-pad a string with a given character.
- `split(text: string, delim: string) -> any`
...
- `trim(text: string [, chars: string]) -> any`
...
- `upper(text: string) -> any`
  - Convert a string to uppercase.
- `word_diff(word1: string, word2: string) -> any`
...
- `fromepoch(timestamp: number) -> any`
