...
- string concatenation, `&` (e.g. `"abc" & "def"` gives `"abcdef"`)
...
- falsey fallback operator, `val1 else val2`. Results in `val1` if val1 is truthy, otherwise results in `val2`. Syntax sugar for `val1 if val1 else val2`.
- truthy fallback operator, `val1 then val2`. Results in `val2` if val1 is truthy, otherwise results in `val1`. Syntax sugar for `val2 if val1 else val2`.
