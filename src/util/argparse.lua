...
local function arg_parse(options)
	local prefixes = {}
	local short = {}
	local long_no_arg = {}
	local long_with_arg = {}
	...
	for i = 1, #arg do
		...
		elseif ar:sub(1, 2) == "--" then
			...
			elseif long_with_arg[ar] then
				...
				if not option then arg_error("Unknown flag `" .. ar .. "`") end
				hold_arg = option.id
...
return {
	...
	parse_and_validate = function(options, config)
		local flags, positional = arg_parse(options)

		if flags.help then
			arg_help(options, config)
			os.exit(0)
		end
		...
		if flags.compile_self then
			...
			if not flags.output then
				arg_error('Must specify an output file with `--output` when using `--compile-self`.')
			end
		...
		if flags.format and (flags.repl or flags.install or flags.plasma_build or flags.cpp_precompile or flags.cpp_clean or flags.introspect or flags.standalone or flags.language_server) then
			arg_warn('Using `--format` outputs formatted code, so any other build process flags are ignored!')
		end
