...
function join(items --[[table]], delimiter --[[string]])
	local result, i = ''
	for i = 1, #items do
...
function strfind(text, substring)
	local i
	for i = 1, #text - #substring + 1 do
		if text:sub(i, i + #substring - 1) == substring then
			return i
